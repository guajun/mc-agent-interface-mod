...
3. Implement the requested behavior, update protocol documentation and capability declarations when needed, and add focused tests when the change warrants them. Keep game-side code independent of Agent/Harness frameworks.
4. Run the relevant tests and build checks available in this repository. Review the complete diff for accidental files, secrets, and unrelated changes. Report checks that could not run and why.
